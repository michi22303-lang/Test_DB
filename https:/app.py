...
st.set_page_config(page_title="CIO Cockpit 7.0 - Dark Mode Edition", layout="wide", page_icon="🏢")
...
with st.sidebar:
    ...
    if dark_mode:
        ...
        card_bg = "#262730" # Dunkles Grau (Streamlit Style)
        text_color = "#ffffff"
        delta_color = "#bdc3c7"
    else:
...
# --- CSS STYLING (DYNAMISCH) ---
def local_css(bg_col, txt_col, delta_col):
    st.markdown(f"""
    <style>
        .block-container {{padding-top: 1rem;}}
        
        /* Karten Design Adaptiv */
        div.css-card {{
...
kpi_func = local_css(card_bg, text_color, delta_color)
...
if selected == "Management Dashboard":
    ...
    if df_proj.empty:
        ...
        if not df_s_curr.empty:
            ...
            with c1: kpi_func("IT-Budget (Ist)", f"{fmt_de(total_budget/1000000, 2, 'M€')}", f"{fmt_de((total_budget-prev_budget)/prev_budget*100, 1, '%')} Wachstum", delta_color)
            with c2: kpi_func("Mitarbeiter (FTE)", f"{fmt_de(fte, 0, '')}", "Köpfe", delta_color)
            with c3: kpi_func("Green IT Score", f"{green_score}/100", "Cloud Efficiency Index", "#00b894") # Grün bleibt grün
            ...
            with col_chart1:
                ...
                fig.update_layout(yaxis_title="Budget (€)", separators=",.", template=plotly_template)
            ...
            with col_chart2:
                ...
                fig_pie.update_layout(showlegend=True, margin=dict(t=0, b=0, l=0, r=0), separators=",.", template=plotly_template)
                st.plotly_chart(fig_pie, use_container_width=True)

# ------------------------------------------------------------------
# TAB 2: MANUELLE PLANUNG (WIZARD)
# ------------------------------------------------------------------
    ...
    if step == 1:
        ...
        with st.form("wiz_step3"):
            ...
            w_scen = st.text_input("Szenario", value="Budget 2026 Manuell")
...
# ------------------------------------------------------------------
    ...
    if df_proj.empty:
        ...
        st.markdown(f"### 1. Treiber einstellen (Basis: {last_actual_year})")
        c1, c2, c3, c4 = st.columns(4)
...
# ------------------------------------------------------------------
# TAB 4: SZENARIO VERGLEICH (FILTER + DARK MODE + SANKEY FIX)
# ------------------------------------------------------------------
    ...
    if df_proj.empty:
        ...
        st.subheader("1. Szenario-Vergleich (Planungsjahre)")
        
        c_sel, c_chart = st.columns([1, 3])
        with c_sel:
            st.info("Zeigt 'Actual' + gewählte Szenarien.")
            comp_sel = st.multiselect("Szenarien:", sim_scens, default=sim_scens[:2] if sim_scens else [])
        
        with c_chart:
            fin_sel = ["Actual"] + comp_sel
            
            # --- FILTER 2023-2025 AUSBLENDEN ---
            # Wir zeigen Actuals erst ab einem sinnvollen Jahr oder nur Planjahre?
            # User Wunsch: "nicht die Jahre 2023 bis 2025 darstellen"
            # Also filtern wir auf year >= 2026 für die Grafik
            
            df_comp = df_proj[df_proj['scenario'].isin(fin_sel)].copy()
            df_comp = df_comp[df_comp['year'] >= 2026] # <--- HIER IST DER FILTER
            
            if df_comp.empty:
                st.warning("Keine Daten ab 2026 gefunden (Simulationen laufen meist auf 2026).")
            else:
                ...
                fig_comp = px.bar(df_grp, x='scenario', y='cost_planned', color='scenario', 
                                  text_auto='.2s', title="Budget Vergleich (ab 2026)",
                                  color_discrete_map={"Actual": "#636e72"}, 
                                  color_discrete_sequence=px.colors.qualitative.Prism)
                fig_comp.update_layout(yaxis_title="Budget (€)", separators=",.", template=plotly_template)
                st.plotly_chart(fig_comp, use_container_width=True)
        
        st.divider()
        
        # --- SANKEY MIT BESSERER LESBARKEIT ---
        ...
        if not df_flow.empty:
            ...
            fig_san = go.Figure(data=[go.Sankey(
                node = dict(
                    ...
                    label = labels, 
                    color = cols,
                    # HIER DIE OPTIMIERUNG: SCHRIFTGRÖSSE & FARBE
                    align = "left",
                    
                ),
                link = dict(source = src, target = tgt, value = val, color = "rgba(100, 100, 100, 0.3)"),
                textfont=dict(size=15, color=text_color, family="Arial Black") # <--- LESBARKEIT & DARK MODE
            )])
            fig_san.update_layout(height=600, title_text=f"Flow: {sel_scen}", template=plotly_template, font=dict(size=14))
...
# ------------------------------------------------------------------
    ...
    if df_proj.empty: st.warning("Keine Daten.")
        ...
        fig_s.update_layout(height=700, separators=",.", template=plotly_template)
...
# ------------------------------------------------------------------
    ...
    if df_proj.empty: st.warning("Keine Daten.")
        ...
        with c1:
            ...
            fig_b.update_layout(template=plotly_template, separators=",.")
...
# ------------------------------------------------------------------
    ...
    with t1:
        if st.button("🚀 Testdaten erzeugen"):
            ...
            projs = []
            for s in stats:
                projs.append({"project_name": "M365 Lizenzen", "category": "Digital Workplace", "opex_type": "Lizenzen", "budget_type": "OPEX", "year": s['year'], "cost_planned": s['fte_count']*1200, "savings_planned": 0, "risk_factor": 1, "strategic_score": 10, "status": "Live", "scenario": "Actual"})
                for i in range(8):
                    projs.append({"project_name": f"Projekt {i}", "category": "Cloud", "budget_type": "OPEX", "year": s['year'], "cost_planned": random.randint(20000, 500000), "savings_planned": 0, "risk_factor": 3, "strategic_score": 5, "status": "Live", "scenario": "Actual"})
